...
async def test_correct_update_user_password(
    ...
    tokens = token_manager.create_tokens(
        authorize=auth_jwt, subject=UserIdData(user_id=user_id)
    )
    ...
    data = UpdateUserData(password="new_password")  # type: ignore
...
async def test_correct_update_user_email(
    ...
    data = UpdateUserData(user_email=EmailStr("qwerty@gmail.com"))  # type: ignore
...
async def test_correct_update_user_email_and_password(
    ...
    data = UpdateUserData(
        password="qwerty1337qwerty", user_email=EmailStr("qwerty@gmail.com")
    )
