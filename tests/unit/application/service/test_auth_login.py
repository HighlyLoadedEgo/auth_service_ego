...
async def test_correct_login(
    ...
    data = LoginSchema(username="test_user", password="qwerty1337")
...
async def test_incorrect_login_with_bad_password(
    ...
    data = LoginSchema(username="test_user", password="incorrect_password")

    with pytest.raises(UserDataIsNotCorrect):
        await auth_service.login_user(data=data, authorize=auth_jwt)
    assert uow.commit_status is False
    assert uow.rollback_status is False
...
async def test_incorrect_login_with_bad_login(
    ...
    data = LoginSchema(username="incorrect_username", password="qwerty1337")
