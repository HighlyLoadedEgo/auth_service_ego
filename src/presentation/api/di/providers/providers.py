...
class InfraProvider:
    ...
    def __init__(
        self, *, pwd_context: CryptContext, pool: async_sessionmaker[AsyncSession]
    ) -> None:
        self._pool = pool
    ...
    async def get_uow(self) -> AsyncGenerator[SQLAlchemyUoW, None]:
        ...
        async with self._pool() as session:
            uow = SQLAlchemyUoW(
                session=session, user_repo=UserRepositoryImpl(session=session)
            )
    ...
    def get_password_manager(self) -> PasswordManagerImpl:
        ...
        """
        pass_manager = PasswordManagerImpl(pwd_context=self._pwd_context)
